    ...
    try:
        ...
        if room.player_a and room.player_b:
            if not Match.objects.filter(room=room).exists():
                ...
                Match.objects.create(board=base_board.fen(), room=room)
    ...
    try:
        ...
        if latest_match:
            ...
            if fullmove_number <= existing_fullmove_number:
                return JsonResponse(
                    {"error": "Incoming turn count is lower than the existing one."},
                    status=409,
                )

        # Proceed to create or update the match
        match = (
            Match.objects.create(room=room, board=fen_string)
            if not latest_match
            else latest_match
        )
